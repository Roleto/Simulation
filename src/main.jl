...
# ------------------------------------------------------------
function run_duffing(q0=10.0, qp0=10.0;
    Adaptive=1,
    Robust=1,
    K=1e5,
    B=-1.0,
    ...
    for i = 1:l
        ...
        u = qDef_pp[i] - αₐ * q[i]
        q_pp[i] = αₑ * q[i] + δₑ * q_p[i] - βₑ * q[i]^3 + u
...
function run_vanderpol(q0=2.0 * sin(0.5 * 1e-3), qp0=2.0 * 0.5 * cos(0.5 * 1e-3);
    ...
    for i = 1:l
        ...
        u = mₐ * qDef_pp[i]
        q_pp[i] = (u + μₑ * (1 - q[i]^2) * q_p[i] - ωₑ^2 * q[i] - αₑ * q[i]^3 - λₑ * q[i]^5) / mₑ
...
function run_lorenz(x0=0.0, y0=0.0, z0=0.0;
    ...
    for i = 1:l
        ...
        u_x = xDef[i] - σₐ * (y[i] - x[i])
        u_y = yDef[i] - x[i] * (ρₐ - z[i]) + y[i]
        u_z = zDef[i] - x[i] * y[i] + βₐ * z[i]

        x_p[i] = σₑ * (y[i] - x[i]) + u_x
        y_p[i] = x[i] * (ρₑ - z[i]) - y[i] + u_y
        z_p[i] = x[i] * y[i] - βₑ * z[i] + u_z
...
function run_rossler(x0=2.0 * sin(0.5 * 1e-3), y0=3.0 * sin(0.7 * 1e-3), z0=1.0 * sin(1.0 * 1e-3);
    ...
    for i = 1:l
        ...
        u_x = xDef[i] + y[i] + z[i]
        u_y = yDef[i] - x[i] - aₐ * y[i]
        u_z = zDef[i] - bₐ - z[i] * (x[i] - cₐ)

        x_p[i] = -y[i] - z[i] + u_x
        y_p[i] = x[i] + aₑ * y[i] + u_y
        z_p[i] = bₑ + z[i] * (x[i] - cₑ) + u_z
        past_response = [x_p[i], y_p[i], z_p[i]]
