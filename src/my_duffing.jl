...
function run_simulation(q_param, q_p_param, q_pp_param)
	...
	# for q0 in range(q_param[1], q_param[2], q_param[3])
	# 	for q_p0 in range(q_p_param[1], q_p_param[2], q_p_param[3])
	# 		max_index+=1
	# 		max_values[max_index] = singlerun(q0, q_p0, false)
	# 	end
	# end
	figure("Error_Max")
	grid(true)
	...
	# 	q_p_mem[1] = q_p0
	# 	q_pp_mem[1] = q_pp0
	# 	singlerun(q_mem, q_p_mem, q_pp_mem, qN_p_mem, qN_pp_mem, u_mem, 1)
	# else
...
using PDFmerger
using Dates
# run_simulation([-1, 1, 0.1], [-1, 1, 0.1], [0, 0, 0])
run_simulation(1, 1, 1)
# run_simulation(1, 5, 2, init_Amp, init_ω, t_range)
# singlerun(q_mem[1], q_p_mem[1], q_pp_mem[1])
